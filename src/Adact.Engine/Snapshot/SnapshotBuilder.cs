...
public sealed class SnapshotBuilder
{
    ...
    public SnapshotBuildResult Build(SnapshotBuildInput input)
    {
        ...
        var rootNode = BuildIncludeNode(input.RootWindow, input.Filter, depth: 0, maxDepth, isModalDialog: false);
        ...
        if (input.ModalSiblings.Count > 0)
        {
            ...
            {
                var modalNode = BuildIncludeNode(modal, input.Filter, depth: 0, maxDepth, isModalDialog: true);
        ...
        return new SnapshotBuildResult(json, _registry.Generation, $"s{_registry.SessionId}");
    }

    /// <summary>Include 確定の要素から JSON ノードを構築する (root / 通常子 共通)。</summary>
    private JsonObject BuildIncludeNode(
        IElement el, IFilterStrategy filter, int depth, int maxDepth, bool isModalDialog)
    {
        var refId = _registry.Register(el);
        ...
        if (depth < maxDepth)
        {
            ...
            {
                BuildChildInto(child, filter, depth + 1, maxDepth, childNodes);
            }
        }
    ...
    private void BuildChildInto(
        IElement child, IFilterStrategy filter, int depth, int maxDepth, JsonArray parentChildren)
    {
        ...
        switch (decision)
        {
            ...
            case NodeDecision.Include:
                parentChildren.Add(BuildIncludeNode(child, filter, depth, maxDepth, isModalDialog: false));
                return;
            case NodeDecision.Flatten:
                if (depth < maxDepth)
                {
                    ...
                    {
                        BuildChildInto(grand, filter, depth + 1, maxDepth, parentChildren);
                    }
                }
                return;
        }
