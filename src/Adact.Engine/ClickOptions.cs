...
/// </summary>
/// <param name="Double">true の場合は OS ダブルクリック判定内で 2 回クリックする (旧 API 互換、Phase 8 では <c>dblclick</c> 推奨)。</param>
/// <param name="Button">押下するボタン。既定は <see cref="MouseButton.Left"/>。</param>
/// <param name="Count">汎用 N 連打回数 (1 以上)。OS ダブルクリック判定は保証しない。</param>
/// <param name="Modifiers">クリック中に押下したままにする修飾キー名 (Shift / Control / Alt / Meta / ControlOrMeta)。null / 空は修飾なし。</param>
