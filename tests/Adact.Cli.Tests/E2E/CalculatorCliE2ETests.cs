...
public class CalculatorCliE2ETests
{
    ...
    public void ListAttachSnapshotClickCloseFlow_OnCalculator_Succeeds()
    {
        ...
        try
        {
            ...
            // 設計 §5.2: stdout に sessionId / windowRef / generation / snapshot 行が出る。
            var sessionId = ExtractKeyValue(attachResult.Stdout, "sessionId");
            Assert.False(string.IsNullOrEmpty(sessionId),
                $"sessionId not found in attach stdout:\n{attachResult.Stdout}");

            var attachGenerationStr = ExtractKeyValue(attachResult.Stdout, "generation");
            Assert.False(string.IsNullOrEmpty(attachGenerationStr),
                $"generation not found in attach stdout:\n{attachResult.Stdout}");
            var attachGeneration = int.Parse(attachGenerationStr!, System.Globalization.CultureInfo.InvariantCulture);
            ...
            // click 後の自動 snapshot は新しい generation でファイル生成される。
            var clickGenerationStr = ExtractKeyValue(clickResult.Stdout, "generation");
            Assert.False(string.IsNullOrEmpty(clickGenerationStr),
                $"generation not found in click stdout:\n{clickResult.Stdout}");
            var clickGeneration = int.Parse(clickGenerationStr!, System.Globalization.CultureInfo.InvariantCulture);
            Assert.True(clickGeneration > attachGeneration,
                $"click snapshot generation should advance: attach={attachGeneration} click={clickGeneration}");
