...
public class AdactDaemonFixtureTests
{
    ...
    {
        using var _ = new EnvironmentVariableScope(AdactDaemonFixture.ServerUrlEnvironmentVariable, null);

        Assert.Null(AdactDaemonFixture.GetExternalServerUrl());
    }
    ...
    public void GetExternalServerUrl_WhenSet_ReturnsTrimmedAbsoluteUrl()
    {
        using var _ = new EnvironmentVariableScope(
            AdactDaemonFixture.ServerUrlEnvironmentVariable,
            "  http://127.0.0.1:41300/mcp  ");

        Assert.Equal("http://127.0.0.1:41300/mcp", AdactDaemonFixture.GetExternalServerUrl());
    }
    ...
    {
        using var _ = new EnvironmentVariableScope(AdactDaemonFixture.ServerUrlEnvironmentVariable, value);

        var ex = Assert.Throws<InvalidOperationException>(() => AdactDaemonFixture.GetExternalServerUrl());
