...
| 変更 | `--snapshot-dir <dir>` |
| 拡張子 | `.txt` |
| 出力する CLI | `attach`, `snapshot`, および auto-snapshot 対象の操作系: `click`, `fill`, `dblclick`, `hover`, `type`, `press`, `check`, `uncheck`, `select`, `clear`, `mouse-wheel`, `resize`, `minimize`, `maximize`, `restore` |
| 出力しない CLI | 低レベル補助 (`mouse-move`, `mouse-down`, `mouse-up`, `key-down`, `key-up`, `focus`, `scroll-into-view`) と取得・同期系 (`inspect`, `screenshot`, `wait-for`, `wait-for-window`, `launch`)。これらは成功時に snapshot を生成しない |
| 抑止 | `--no-snapshot` を持つコマンド (`attach`, `click`, `fill`, `dblclick`, `hover`, `type`, `press`, `check`, `uncheck`, `select`, `clear`, `mouse-wheel`, `resize`, `minimize`, `maximize`, `restore`) で自動 snapshot を抑止できる |
