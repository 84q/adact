...
| 5. MCP tool | `WindowsTools` | `windows_*` または `adact_daemon_stop` を実行する |
...
4. CLI は MCP `adact_list_windows` を呼びます。
...
2. CLI は MCP `adact_attach` に `windowRef` を渡します。
...
9. `--no-snapshot` がなければ、続けて `CommandHelpers.WriteSnapshotResultAsync()` が `adact_snapshot` を呼び、snapshot path を出力します。

## `adact snapshot`

| 観点 | 内容 |
| --- | --- |
| CLI tool | `adact_snapshot` |
...
2. `CommandHelpers.WriteSnapshotResultAsync()` が filter 名を検証し、MCP `adact_snapshot` を呼びます。
...
sequenceDiagram
	...
	CLI->>Client: adact_click or adact_fill
	Client->>Host: MCP request
	...
	opt auto snapshot
		CLI->>Cmd: WriteSnapshotResultAsync
		Cmd->>Client: adact_snapshot for session
		Client->>Host: MCP request
		Host->>Tools: SnapshotAsync
	end
```
...
| MCP tool | `adact_click` | `adact_fill` |
...
3. CLI は MCP `adact_click` または `adact_fill` を呼びます。
...
9. `--no-snapshot` がなければ CLI が `adact_snapshot` を追加で呼び、操作後の snapshot path を出力します。`--no-snapshot` の場合は最低限 `sessionId` を出力します。

## `adact wait-for-element`

| 観点 | 内容 |
| --- | --- |
| MCP tool | `adact_wait_for_element` |
...
2. CLI は MCP `adact_wait_for_element` に必要な引数だけを渡します。検索条件モードでは `--sid` も透過します。
...
| 観点 | 内容 |
| --- | --- |
| MCP tool | `adact_wait_for_window` |
...
2. CLI は MCP `adact_wait_for_window` を呼びます。
...
2. CLI は `adact_detach` / `adact_close_window` / `adact_kill` を呼びます。
...
3. CLI は MCP `adact_daemon_stop` を呼びます。
